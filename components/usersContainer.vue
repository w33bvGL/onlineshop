<template>
  <div class="bg-white w-full rounded-2xl p-6">
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <div
          class="input-width pt-3 border-2 border-gray-200 pb-3 rounded-2xl bg-gray-100 outline-none relative w-full hidden lg:block">
        <input
            v-model="searchQuery"
            type="text"
            required
      ...
      <span class="text-custom-green-1100 text-nowrap pl-10">
        {{ filtered.length }} пользователь{{ filtered.length > 1 ? 'я' : '' }}
      </span>
    </div>
    <div
        class="input-width pt-3 border-2 border-gray-200 pb-3 rounded-2xl bg-gray-100 outline-none relative w-full block lg:hidden">
      <input
          v-model="searchQuery"
          type="text"
          required
    ...
    <div class="sort mt-4">
      <div>
        <ul class="font-medium text-lg flex gap-4 flex-nowrap flex-row overflow-x-auto text-nowrap pb-2 pt-2 skip-scrollbar cursor-pointer">
          ...
          <li class="border-2 pr-2 pl-2 rounded-xl"
              :class="{ 'border-gray-300': selectedFilter === 'partnersCount', 'opacity-45': selectedFilter !== 'partnersCount' }"
    ...
    <div>
      <div v-for="user in filtered" :key="user.id" class="cursor-pointer">
        <div class="mt-3">
